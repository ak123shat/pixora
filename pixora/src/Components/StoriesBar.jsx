...
const StoriesBar = () => {
    const [stories , setStories] = useState([]); // Assuming stories will be fetched or passed as props
    const [showModel , setShowModel] = useState(false);
    const [viewStory , setViewStory] = useState(null);
    const fetchStories = async () => {
        setStories(dummyStoriesData); // Replace with actual data fetching logic
    }
    useEffect(() => {
        fetchStories();
    } , [])
                        ...
                        story.media_type !== 'text' && (
                            <div className='absolute inset-0 z-1 rounded-lg bg-black overflow-hidden'>
                                {story.media_type === 'image' ? (
                                    ...
                                    <video src={story.media_url} className='w-full h-full object-cover hover-scale-110 transition-duration-500 opacity-70 hover:opacity-80' controls />
                                )}
                            </div>
                        )
