...
//import {UserButton} from '@radix-ui/react-user';

const SideBar = ({sidebarOpen, setSidebarOpen}) => {
    const navigate = useNavigate();
    const user = dummyUserData; // Replace with actual user data fetching logic
    const {SignOut} = useUser(); // Assuming useUser is a custom hook for user management
  return (
    ...
    ${sidebarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      ...
      <div className='w-full'>
        ...
        <img onClick={()=> navigate('/')} src={logo} alt="Logo" className='w-26 ml-7 my-2 cursor-pointer' />
        ...
        <Link to = '/create-post' className='flex items-center justify-center gap-2 py-2.5 mt-6 mx-6 rounded-lg
            bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95 transition text-white cursor-pointer'>
